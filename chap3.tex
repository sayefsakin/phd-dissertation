...
\addfigure[width=\columnwidth, alt={Gantt View representing parent-child dependency between two intervals. The highlighted yellow interval bar $b$ has parent $a$. Interval $b$ has three children\textemdash $c$, $d$, and $e$.}]{figs/gantt.jpg}{Gantt View representing parent-child dependency between two intervals. The highlighted yellow interval bar $b$ has parent $a$. Interval $b$ has three children\textemdash $c$, $d$, and $e$.}{fig:gantt}
...
\addfigure[width=\textwidth, alt={Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View that it is executing alone as highlighted in the Utilization, Gantt, and Aggregated Gantt view.}]{figs/halide_a1.JPG}{Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as highlighted in all temporal views (a, b, c).}{fig:casestudy31}
...
\addfigure[width=\textwidth, alt={Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as highlighted in the Utilization, Gantt, and Aggregated Gantt view. After we identified and fixed this bug, we are able to create multiple variables concurrently.}]{figs/halide_p1.JPG}{Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as highlighted in all temporal views (a, b). After we identified and fixed this bug, we are able to create multiple variables concurrently.}{fig:casestudy32}
