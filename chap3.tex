...
\chapter[Traveler: Navigating Task Parallel Traces for Performance Analysis]{Traveler: Navigating Task Parallel \\ Traces for Performance Analysis}
\label{chap:traveler}

\begin{refsection}
This work is based on previously published work, \copyright  2022 IEEE, Reprinted with permission from S. A. Sakin, A. Bigelow, R. Tohid, C. Scully-Allison, C. Scheidegger, S. R. Brandt, C. Taylor, K. A. Huck, H. Kaiser, and K. E. Isaacs, ``Traveler: Navigating task parallel traces for performance analysis,'' \emph{IEEE Trans. Vis. Comput. Graph.}, vol. 29, no. 1, pp. 788-797, 2022.
%  This work was published as a full paper~\cite{sakin2022traveler} at IEEE VIS 2022. The combined work was published at the Practice and Experience in Advanced Research Computing Conference 2020~\cite{brandt2020jetlag}, 2020 IEEE ACM 9th Workshop on Python for High-Performance and Scientific Computing~\cite{brandt2020distributed}, and Euro-Par 2022 Parallel Processing Workshops~\cite{tohid2022halide}. The published Traveler~\cite{sakin2022traveler} paper has been slightly modified and presented in this chapter.

\section{Introduction}
The first step toward even sequence visualization is understanding the visual design challenges. To investigate the challenges, specifically for visualizing the vast scale differences and facilitating interactive navigation of the data, we collaborated with HPC researchers to understand their performance analysis needs. In the completed work, Traveler, we conducted a visualization design study and developed a highly linked multi-view coordinated visualization platform for analyzing task parallel traces. Primarily focused on Gantt charts, Traveler provides multiple linked auxiliary interfaces to help navigate from multiple contexts of the underlying performance data generated from parallel computing systems. This work provides a foundation for developing solutions for scalable Gantt charts by identifying the visual design needs of our collaborators.
...
\addfigure[width=\linewidth, alt={The Traveler interface. Traveler is a visualization platform supporting visual performance analysis of asynchronous task parallel programs. The Utilization View shows that the selected primitive (task type) accounts for most of the utilization in the program. The Gantt View shows how individual tasks are scheduled across the system, while the Aggregated Gantt View shows utilization and extent due to groups of tasks. A Functional Box Plot View summarizes performance metrics across resources and over time. Selection details are shown in the Selection Info View and source code in the Source Code View. The Dependency Tree View shows relations among task types. The distribution of task durations can be explored in the Interval Histogram View.}]{figs/traveler_teaser_new.jpg}{The Traveler interface. Traveler is a visualization platform supporting visual performance analysis of asynchronous task parallel programs. The Utilization View (a) shows that the selected primitive (task type) accounts for most of the utilization in the program. The Gantt View (b) shows how individual tasks are scheduled across the system, while the Aggregated Gantt View (c) shows utilization and extent due to groups of tasks. A Functional Box Plot View (d) summarizes performance metrics across resources and over time. Selection details are shown in the Selection Info View (e) and source code in the Source Code View (f). The Dependency Tree View (g) shows relations among task types. The distribution of task durations can be explored in the Interval Histogram View (h).}{fig:travelerteaser}
...
\addfigure[width=0.9\columnwidth, alt={Aggregated Gantt view on the left shows one bar by combining all subsequent events initiated from a single event. Dependency Tree View on the right shows the node-link diagram generated from the dependencies among the events.}]{figs/agg_gantt_depp.png}{Visualizing summerized events with dependencies in Traveler. Aggregated Gantt (Left) view shows one bar by combining all subsequent events initiated from a single event. Dependency Tree View (Right) shows the node-link diagram generated from the dependencies among the events.}{fig:agg_depp}
...
%Kevin asked to remove this paragraph
...
\addfigure[width=0.7\columnwidth, alt={A goal-to-task lattice, showing relationships between high-level umbrella concerns, goals, sub-goals, tasks, and the task taxonomies. Additional goals, sub-goals, tasks, and frameworks, relative to prior work, are shown in yellow.}]{figs/lattice3.pdf}{A goal-to-task lattice, showing relationships between high-level umbrella concerns, goals, sub-goals, tasks, and the task taxonomies. Additional goals, sub-goals, tasks, and frameworks, relative to prior work, are shown in yellow.}{fig:taskanalysis}

% \begin{figure}[t]
%   \centering
%   \includegraphics[width=0.5\columnwidth]{figs/lattice3.pdf}
...
The revised lattice (\autoref{fig:taskanalysis}) retained three umbrella concerns: \textbf{U1: Program Comprehension}, \textbf{U2: Performance Analysis}, and \textbf{U3: Communication}. Seven design goals were articulated under these concerns:
...
% \begin{figure}[t]
%     \centering
%     \includegraphics[width=\columnwidth]{figs/gantt.jpg}
...
\addfigure[width=\columnwidth, alt={Gantt View representing parent-child dependency between two intervals. The highlighted yellow interval bar marked with b has parent marked with a. Interval marked with b has three children marked with c, d, and e.}]{figs/gantt.jpg}{Gantt View representing parent-child dependency between two intervals. The highlighted yellow interval bar $b$ has parent $a$. Interval $b$ has three children\textemdash $c$, $d$, and $e$.}{fig:gantt}
...
\addfigure[width=\columnwidth, alt={Aggregated Gantt view showing combined intervals from a single primitive's subtree. Dependency Tree View shows the node-link diagram generated from the dependencies among the events.}]{figs/AggGanttExplainer20.png}{Primitive subtree highlighting. Selecting a primitive in the Dependency Tree View will highlight instances of that primitive's subtree in the Gantt View and create aggregated bars for the subtree in the Aggregated Gantt View.}{fig:agg_gantt}
...
The Functional Box Plot view \autoref{fig:travelerteaser}(d) summarizes performance counter rates across resources. We draw three lines representing the maximum value across resources, the minimum, and the average. A shaded gray area around the average shows a standard deviation. \autoref{fig:travelerteaser}(d) shows the Functional Box Plot view for the  PAPI (Performance Application Programming Interface)~\cite{browne2000portable} metric CPU cycles. 
...
\addfigure[width=\textwidth, alt={The distribution of intervals in Phylanx-Blaze. The Selection Info View shows the exact duration of the selected bars while the Gantt View shows where they occur in time and on what resources. The Utilization View shows their impact compared to the whole program. The Interval Histogram View shows the longest intervals which are selected via brush.}]{figs/blaze.JPG}{The distribution of intervals in Phylanx-Blaze. The Selection Info View (a) shows the exact duration of the selected bars while the Gantt View (b) shows where they occur in time and on what resources. The Utilization View (c) shows their impact compared to the whole program. The Interval Histogram View (d) shows the longest intervals which are selected via brush.}{fig:casestudy11}

% \begin{figure}[t]
%     \includegraphics[width=\textwidth]{figs/blaze.JPG}
%     \caption{The distribution of intervals in Phylanx-Blaze. The Selection Info View (a) shows the exact duration of the selected bars while the Gantt View (b) shows where they occur in time and on what resources. The Utilization View (c) shows their impact compared to the whole program. The Interval Histogram View (d) shows the longest intervals which are selected via brush.
%     }
%     \label{fig:casestudy11}
% \end{figure}

\addfigure[width=\textwidth, alt={The distribution of intervals in Phylanx-Halide. The Selection Info View shows the exact duration of the selected bars while the Gantt View shows where they occur in time and on what resources. The Utilization View shows their impact compared to the whole program. The Interval Histogram View shows the longest intervals which are selected via brush.}]{figs/halide.JPG}{The distribution of intervals in Phylanx-Halide. The Selection Info View (a) shows the exact duration of the selected bars while the Gantt View (b) shows where they occur in time and on what resources. The Utilization View (c) shows their impact compared to the whole program. The Interval Histogram View (d) shows the longest intervals which are selected via brush.}{fig:casestudy12}

% \begin{figure}[t]
%     \includegraphics[width=\textwidth]{figs/halide.JPG}
%     \caption{The distribution of intervals in Phylanx-Halide. The Selection Info View (a) shows the exact duration of the selected bars while the Gantt View (b) shows where they occur in time and on what resources. The Utilization View (c) shows their impact compared to the whole program. The Interval Histogram View (d) shows the longest intervals which are selected via brush.
%     }
%     \label{fig:casestudy12}
% \end{figure}

\subsubsection{Investigation with Traveler}
\label{subsubsec:investigation}
Viewing both execution traces in Traveler, we opened the Interval Histogram View to compare (Goal G3.4) the distributions in interval durations (G3.5). The Phylanx-Blaze (\autoref{fig:casestudy11}) distribution is much wider, with many intervals longer than the longest ones in Phylanx-Halide (\autoref{fig:casestudy12}), indicating some tasks are taking much longer in Phylanx-Blaze. We select the long intervals in both traces (G3.6). The highlighted intervals in the Gantt show us the tasks are shorter and more evenly distributed in Phylanx-Halide. The Selection Info View reveals the extreme Phylanx-Blaze interval is six times longer than the longest one in Phylanx-Halide, contributing to the longer execution time. {\em Insight:} Phylanx-Blaze intervals require more time for the same work and are more unbalanced.

The RTM suspects that there may be data movement differences because data management is a feature of Halide. We open multiple Functional Box Plot views to check the L1 and L2 Cache miss rates (sub-goal CC.2). A higher miss rate suggests more time is being spent reading from slower memory. We brush over the Utilization View to focus on the second area of high activity in the computation. We see that while the fluctuations in L1 cache misses are similar, the Phylanx-Blaze version has a steadily increasing rate of L2 cache misses while the Phylanx-Halide version remains flat. {\em Insight:} the Phylanx-Halide version is managing data more effectively as suspected.

\addfigure[width=\textwidth, alt={Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive marked with A in the Dependency Tree View that it is executing alone as highlighted in the Utilization, Gantt, and Aggregated Gantt view.}]{figs/halide_a1.JPG}{Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as highlighted in all temporal views (a, b, c).}{fig:casestudy31}

% \begin{figure}[t]
%     \includegraphics[width=\textwidth]{figs/halide_a1.JPG}
%     \caption{Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as show in all temporal views (a, b, c).
%     }
%     \label{fig:casestudy31}
% \end{figure}

\addfigure[width=\textwidth, alt={Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive marked with A in the Dependency Tree View that it is executing alone as highlighted in the Utilization, Gantt, and Aggregated Gantt view. After we identified and fixed this bug, we are able to create multiple variables concurrently.}]{figs/halide_p1.JPG}{Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as highlighted in all temporal views (a, b). After we identified and fixed this bug, we are able to create multiple variables concurrently.}{fig:casestudy32}

% \begin{figure}[t]
%     \includegraphics[width=\textwidth]{figs/halide_p1.JPG}
%     \caption{Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as highlighted in all temporal views (a, b). After we identified and fixed this bug, we are able to create multiple variables concurrently.
%     }
%     \label{fig:casestudy32}
% \end{figure}

During the previous analyses, we noted the long low-utilization periods between the two high-utilization phases of the program in the Utilization View (Goal G5). We used the Dependency Tree View to understand the mapping between primitives near the root (high level operations) and utilization (sub-goal CC.3). As shown in \autoref{fig:casestudy31}, some of the low utilization can be attributed solely to the declaration of the variable $A$, as seen with the yellow area in the utilization view. We repeated the process with neighboring nodes for variables $B$ and $C$---each was the only active primitive in a separate area of low utilization. {\em Insight:} The declarations are serialized.
...
After updating the code, we regenerated the OTF2 data, resulting in \autoref{fig:casestudy32}. The smaller gap between the two peaks in the utilization view verifies that the declaration of the variables $A$, $B$, and $C$ occurred in parallel. This bug fix was merged into the Phylanx repository.
...
\textbf{First, leveraging an existing task analysis while adapting to shifting data helped with evolving design requirements.} The task analysis presented in \autoref{sec:traveler-tasks} builds upon an earlier effort conducted at the beginning of the collaboration, when our focus was on execution tree profile data\textemdash data aggregated by primitive context. As before, we tracked tasks longitudinally, reinforcing those that recurred over time while de-emphasizing tasks of limited relevance. The higher-level umbrella concerns and goals remained largely stable, but numerous new sub-goals emerged. This relative stability at the goal level gave us confidence in the prioritization of design features, particularly those that expanded support for existing goals when new data became available. Moreover, the persistence of prior goals enabled us to transfer components of earlier visualization designs into Traveler, guiding decisions about which features to port and which to modify.  
