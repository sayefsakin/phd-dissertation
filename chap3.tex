...
\addfigure[width=\linewidth]{figs/traveler_teaser_new.jpg}{The Traveler interface. Traveler is a visualization platform supporting visual performance analysis of asynchronous task parallel programs. The Utilization View (a) shows that the selected primitive (task type) accounts for most of the utilization in the program. The Gantt View (b) shows how individual tasks are scheduled across the system, while the Aggregated Gantt View (c) shows utilization and extent due to groups of tasks. A Functional Box Plot View (d) summarizes performance metrics across resources and over time. Selection details are shown in the Selection Info View (e) and source code in the Source Code View (f). The Dependency Tree View (g) shows relations among task types. The distribution of task durations can be explored in the Interval Histogram View (h).}{fig:travelerteaser}
...
\addfigure[width=0.7\columnwidth]{figs/lattice3.pdf}{A goal-to-task lattice, showing relationships between high-level umbrella concerns, goals, sub-goals, tasks, and the task taxonomies. Additional goals, sub-goals, tasks, and frameworks, relative to prior work, are shown in yellow.}{fig:taskanalysis}
...
\addfigure[width=\columnwidth]{figs/gantt.jpg}{Gantt View representing parent-child dependency between two intervals. The highlighted yellow interval bar $b$ has parent $a$. Interval $b$ has three children\textemdash $c$, $d$, and $e$.}{fig:gantt}
...
\addfigure[width=\columnwidth]{figs/AggGanttExplainer20.png}{Primitive subtree highlighting. Selecting a primitive in the Dependency Tree View will highlight instances of that primitive's subtree in the Gantt View and create aggregated bars for the subtree in the Aggregated Gantt View.}{fig:agg_gantt}
...
\addfigure[width=\textwidth]{figs/halide.JPG}{The distribution of intervals in Phylanx-Halide. The Selection Info View (a) shows the exact duration of the selected bars while the Gantt View (b) shows where they occur in time and on what resources. The Utilization View (c) shows their impact compared to the whole program. The Interval Histogram View (d) shows the longest intervals which are selected via brush.}{fig:casestudy12}
...
\addfigure[width=\textwidth]{figs/halide_a1.JPG}{Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as show in all temporal views (a, b, c).}{fig:casestudy31}
...
\addfigure[width=\textwidth]{figs/halide_p1.JPG}{Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as show in all temporal views (a, b). After we identified and fixed this bug, we are able to create multiple variables concurrently.}{fig:casestudy32}

% \begin{figure}[t]
%     \includegraphics[width=\textwidth]{figs/halide_p1.JPG}
%     \caption{Traveler helped us identify a performance bug related to serialization of variable instantiation leading to a fix that shortens execution time. We select a primitive $A$ in the Dependency Tree View (d) that it is executing alone as show in all temporal views (a, b). After we identified and fixed this bug, we are able to create multiple variables concurrently.
%     }
