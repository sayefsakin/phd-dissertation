...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\columnwidth]{figs/agglokdt.png}
...
Starting from a track-level root, we recursively split any node whose time span covers more than two events. We adopt a \emph{fair} splitting rule that partitions the node's events into two children with (as nearly as possible) equal counts, ordered temporally. The \emph{fair} splitting rule yields a balanced search depth and, in our preliminary testing, strong pixel-level accuracy when summarization is set to one pixel. Internal nodes store a concise summary of their covered events; leaves store individual events. See \ref{fig:hierarchy} for an illustration.

\textbf{Two-dimensional KD-Tree to summarize all events.}
To simultaneously partition time and tracks, we construct a two-dimensional KD-tree whose root spans the full chart. Splits alternate between the temporal and track axes. For time splits, we use the midpoint between the earliest start and latest end within the parent; if that midpoint intersects events, those events are split across children; if not, child bounds are snapped to the nearest event extents to avoid empty regions. For track splits, we divide the current track set as evenly as possible; once a node contains a single track, splitting reverts to the 1D fair rule. \ref{fig:twodkdt} shows the resulting partitioning, with boundary-defining events highlighted.

\textbf{Agglomerative clustering to summarize events per track.}

As a bottom-up alternative, we cluster events along a track using single-linkage on the temporal gap between events. Clusters merge greedily by proximity, producing compact temporal groups; each cluster's summary is defined by the earliest start and latest end of its members. The resulting dendrogram provides a hierarchical basis analogous to the KD-tree, but formed by data-driven cohesion rather than axis-aligned partitioning; \ref{fig:hierarchy} depicts the structure.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figs/2dkdt-02.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\columnwidth]{figs/window_data_fetch_comparison.png}
    \caption{Comparison of average fetch time (top) and visual accuracy (bottom) across seven data management configurations for range queries. The dashed line marks the 100ms interactivity threshold. ESeMan configurations reduce average fetch time while preserving high visual accuracy.}
    \label{fig:rangeqb}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\columnwidth]{figs/window_total_drawing_relative_increase.png}
    \caption{Comparison of avergage rendering time (top) and percent increase from the lowest rendering time (bottom) across seven data management configurations for range queries. ESeMan configurations outperform alternatives in rendering time.}
    \label{fig:rangeqc}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\columnwidth]{figs/cond_stacked_chart.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\columnwidth]{figs/cond_data_fetch_comparison.png}
    \caption{Comparison of average fetch time (top) and visual accuracy (bottom) across seven data management configurations for conditional range queries. The dashed line marks the 100ms interactivity threshold. ESeMan configurations reduce average fetch time while preserving high visual accuracy.}
    \label{fig:crangeqb}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\columnwidth]{figs/cond_total_drawing_relative_increase.png}
    \caption{Comparison of avergage rendering time (top) and percent increase from the lowest rendering time (bottom) across seven data management configurations for conditional range queries. ESeMan configurations outperform alternatives in rendering time.}
    \label{fig:crangeqc}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figs/esemanteaser.png}
    \caption{Parallel timeline charts constructed from 160K events distributed across 49 processors (rows) of a parallel program. Gray boxes with dark borders represent single event. (a) shows a naive rendering of all data, (b - d) show renderings using ESeMan with varying summarization levels. The naive solution requires 499 milliseconds (ms) for the data fetch, which is outside best practices for interactivity. ESeMan retrieves data for an equivalent chart in one tenth the time and can be tuned to trade off accuracy for fetch time. In (c), a 16-pixel summarization window reduces fetch time by half with noticeable visual differences (quantified by the SSIM score). In (d), a 32-pixel window further decreases fetch time with further degradation to accuracy.}
    \label{fig:teasers}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figs/listings51.png}
    \caption{C3 - Naive Range Query}
    \label{fig:listings51}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figs/listings52.png}
    \caption{C4 - Statistical Sub-sampling Query}
    \label{fig:listings52}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figs/listings53.png}
    \caption{C5 - M4 Optimization Query}
    \label{fig:listings53}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Indexing time for each dataset across data management configurations.}
...
\begin{table}[t]
\centering
\caption{Indexed file size for each dataset across data management configurations.}
...
\begin{figure}[t]
    ...
    \caption{Initial average data fetch time across datasets}
...
\begin{tabular}{|c|c|}
\hline
\textbf{Generated Visualization} & \textbf{Configuration} (SSIM) \\
\hline
