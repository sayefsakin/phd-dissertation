%%% -*-LaTeX-*-
\chapter[Managing Data for Scalable and Interactive Event Sequence Visualization]{Managing Data for Scalable and Interactive \\ Event Sequence Visualization}
\label{chap:eseman}

\begin{refsection}
This work is based on previously published work, \copyright  2025 IEEE, Reprinted with permission from S. A. Sakin and K. E. Isaacs, ``Managing Data for Scalable and Interactive Event Sequence Visualization,'' in \emph{2025 IEEE 15th Symp. Large Data Anal. Vis. (LDAV)}, IEEE, 2025, pp. 33-43.

\section{Introduction}
\label{sec:eseman_intro}

Event sequence data arises across a wide variety of domains, including healthcare, manufacturing, and high-performance computing (HPC). Such data capture temporally ordered events along multiple tracks or entities, often resulting in millions of records. A widely used visualization idiom for such data is the parallel timeline chart, where each event sequence is drawn on a horizontal track and events are rendered as bars positioned according to their timestamps. These charts provide rich context but encounter serious scalability limitations as data size increases. When analysts attempt to zoom, pan, or filter large event datasets, interactive performance is often lost due to the overwhelming cost of retrieving and rendering all relevant events.
...
\addfigure[width=1.0\columnwidth, alt={Illustration of a parallel timeline chart showing three tracks of event sequences across a selected time window.}]{figs/ganttoverview.png}{Illustration of a parallel timeline chart showing three tracks of event sequences across a selected time window.}{fig:eseman_gantt_desc}
...
\addfigure[width=\columnwidth, alt={ESeMan pipeline. Raw events are cleaned and serialized, then indexed hierarchically across time and tracks. A query server responds to requests from the visualization, returning summarized events that are sufficient to render at the current scale. The pixel window parameter adjusts the visual accuracy and latency.}]{figs/framework.png}{ESeMan pipeline. Raw events are cleaned and serialized, then indexed hierarchically across time and tracks. A query server responds to requests from the visualization, returning summarized events that are sufficient to render at the current scale. The pixel window parameter adjusts the visual accuracy and latency.}{fig:pipeline}
...
Two deployment patterns are supported: a \emph{forest} of per-track trees (useful when track ordering is frequently reconfigured or filtered) and a \emph{single 2D tree} over the joint time-track domain (appropriate when track order is fixed or reordering is not a common interaction). Across these patterns, ESeMan offers KD-tree variants and an agglomerative clustering option; their comparative behaviors are analyzed in \ref{sec:eval}.

\addfigure[width=0.8\columnwidth, alt={Hierarchical organizations for a single track. During queries, once a node corresponds to the spatial-temporal footprint of a pixel window, the node's summary (not its individual events) is returned, yielding a resolution-appropriate depiction. The top graph shows per-track one dimensional KD-Tree where internal nodes hold summaries and leaves hold events. The bottom graph shows per-track agglomerative hierarchy where clusters summarize proximate events.}]{figs/agglokdt.png}{Hierarchical organizations for a single track. During queries, once a node corresponds to the spatial-temporal footprint of a pixel window, the node's summary (not its individual events) is returned, yielding a resolution-appropriate depiction.}{fig:hierarchy}

\addfigure[width=\columnwidth, alt={Two-dimensional KD-tree spanning time and tracks. Internal nodes summarize multi-track regions; when a pixel window maps to a node containing multiple events, only the node's temporal (and track) bounds are returned, forming a faithful summary mark.}]{figs/2dkdt-02.png}{Two-dimensional KD-tree spanning time and tracks. Internal nodes summarize multi-track regions; when a pixel window maps to a node containing multiple events, only the node's temporal (and track) bounds are returned, forming a faithful summary mark.}{fig:twodkdt}

\subsubsection{One-Dimensional KD-Tree to Summarize Events per Track}

Starting from a track-level root, we recursively split any node whose time span covers more than two events. We adopt a \emph{fair} splitting rule that partitions the node's events into two children with (as nearly as possible) equal counts, ordered temporally. The \emph{fair} splitting rule yields a balanced search depth and, in our preliminary testing, strong pixel-level accuracy when summarization is set to one pixel. Internal nodes store a concise summary of their covered events; leaves store individual events. See \autoref{fig:hierarchy} for an illustration.

\subsubsection{Two-Dimensional KD-Tree to Summarize All Events}
...
\subsubsection{Agglomerative Clustering to Summarize Events per Track}

As a bottom-up alternative, we cluster events along a track using single-linkage on the temporal gap between events. Clusters merge greedily by proximity, producing compact temporal groups; each cluster's summary is defined by the earliest start and latest end of its members. The resulting dendrogram provides a hierarchical basis analogous to the KD-tree, but formed by data-driven cohesion rather than axis-aligned partitioning; \autoref{fig:hierarchy} depicts the structure.

\subsection{Summarizing Event Attribute Data}
\label{subsec:attributes}

Beyond temporal extent, events carry attributes used for labeling, coloring, and filtering. After the index is constructed, ESeMan performs a bottom-up pass to attach attribute summaries to internal nodes. For \emph{categorical} attributes, nodes store the set of distinct values appearing in their subtree in a hash-based structure keyed by attribute name. Our design supports fast merges during index construction and fast lookups at query time without requiring client-side callbacks. While bitmap encodings can reduce memory, they impose fixed universes and ship decoding state to the client; we therefore favor hash-sets for flexibility.
...
\addtable{Characteristics of datasets used in the evaluation.}{tab:datasets}{|l|c|c|c|}{
  HLINE
  BOLD Dataset & BOLD Distribution & BOLD \# Tracks & BOLD \# of Events \\
  HLINE
...
\addfigure[width=0.9\columnwidth, alt={Percentage breakdown of total time spent on data fetching (dark bars) and chart rendering (light bars) across seven data management configurations for range queries. Except for the LRA and synthesized dataset, data fetch time dominates total time for the non-ESeMan configurations. ESeMan configurations substantially lowers the data fetch percentage.}]{figs/window_stacked_chart.png}{Percentage breakdown of total time spent on data fetching (dark bars) and chart rendering (light bars) across seven data management configurations for range queries. Except for the LRA and synthesized dataset, data fetch time dominates total time for the non-ESeMan configurations. ESeMan configurations substantially lowers the data fetch percentage.}{fig:rangeqa}
...
\addfigure[width=0.9\columnwidth, alt={Comparison of average fetch time, presented at the top, and visual accuracy, presented at the bottom, across seven data management configurations for range queries. The dashed line marks the 100ms interactivity threshold. ESeMan configurations reduce average fetch time while preserving high visual accuracy.}]{figs/window_data_fetch_comparison.png}{Comparison of average fetch time (top) and visual accuracy (bottom) across seven data management configurations for range queries. The dashed line marks the 100ms interactivity threshold. ESeMan configurations reduce average fetch time while preserving high visual accuracy.}{fig:rangeqb}
The percentage breakdown of total time between data fetching and chart rendering across datasets and configurations is presented in~\autoref{fig:rangeqa} for range queries. Comparatively darker bars represent data fetch time and comparatively lighter bars depict chart rendering time for each configuration. The non-ESeMan configurations spend a dominant portion of time on data fetching for DGEMM, K-Means, LULESH, and Fibonacci dataset. ESeMan configurations consistently reduce the data fetch percentage accross datasets, except for Fibonacci dataset. Next, individual results for average data fetch and chart rendering time are presented.

\addfigure[width=0.9\columnwidth, alt={Comparison of avergage rendering time, presented at the top, and percent increase from the lowest rendering time, presented at the bottom, across seven data management configurations for range queries. ESeMan configurations outperform alternatives in rendering time.}]{figs/window_total_drawing_relative_increase.png}{Comparison of avergage rendering time (top) and percent increase from the lowest rendering time (bottom) across seven data management configurations for range queries. ESeMan configurations outperform alternatives in rendering time.}{fig:rangeqc}

\autoref{fig:rangeqb} presents the average data fetch times (top) and visual accuracy (bottom) for range queries. The ESeMan variants consistently achieved latency below the 100ms interactivity threshold~\cite{battle2019role}, except on the largest synthesized dataset where times reached 400ms. The \texttt{Agg} variant exhibited similar behavior, except for on the Fibonacci dataset where it crashed due to a memory allocation error. The naive SQL and approximate methods frequently exceeded the threshold, particularly for dense or large traces.

In terms of accuracy, ESeMan maintained near-perfect similarity with the naive baseline, whereas subsampling and M4 optimization introduced visible distortions. Among ESeMan\'s variants, \texttt{1DKDT} achieved the fastest performance, while both \texttt{1DKDT} and \texttt{Agg} provided the highest fidelity.

\addfigure[width=0.9\columnwidth, alt={Percentage breakdown of total time spent on data fetching (dark bars) and chart rendering (light bars) across seven data management configurations for conditional range queries. Except for the LULESH, LRA, and synthesized dataset, data fetch time dominates time for the non-ESeMan configurations. ESeMan configurations substantially lowers the data fetch percentage.}]{figs/cond_stacked_chart.png}{Percentage breakdown of total time spent on data fetching (dark bars) and chart rendering (light bars) across seven data management configurations for conditional range queries. Except for the LULESH, LRA, and synthesized dataset, data fetch time dominates time for the non-ESeMan configurations. ESeMan configurations substantially lowers the data fetch percentage.}{fig:crangeqa}

\autoref{fig:rangeqc} presents the average chart rendering times (top) and percent increase from the lowest rendering time per dataset (bottom) for range queries. The chart rendering time remains comparatively stable across configurations for similar SSIM values where the rendering workload remains similar (\autoref{fig:rangeqc}(bottom)). Except for the LULESH dataset, at least one EseMan configurations achieves the lowest rendering time across dataset, whereas for LULESH the Naive remains the fastest. For fibonacci dataset, ESeMan configurations have the fastest rendering time which is also lower than the data fetch time (\autoref{fig:rangeqb}(top)) that results into higher data fetch percentage in \autoref{fig:rangeqa}.

\subsubsection{Conditional Range Query Performance}

\addfigure[width=0.9\columnwidth, alt={Comparison of average fetch time, presented at the top, and visual accuracy, presented at the bottom, across seven data management configurations for conditional range queries. The dashed line marks the 100ms interactivity threshold. ESeMan configurations reduce average fetch time while preserving high visual accuracy.}]{figs/cond_data_fetch_comparison.png}{Comparison of average fetch time (top) and visual accuracy (bottom) across seven data management configurations for conditional range queries. The dashed line marks the 100ms interactivity threshold. ESeMan configurations reduce average fetch time while preserving high visual accuracy.}{fig:crangeqb}

For conditional range queries, \autoref{fig:crangeqa} presents the percentage breakdown of total time between data fetching and chart rendering across datasets and configurations. Similarly, comparatively darker and lighter bars are utilized to present data fetch and chart rendering time respectively. The non-ESeMan configurations spend a dominant portion of time on data fetching for DGEMM, K-Means, and Fibonacci dataset. ESeMan configurations consistently reduce the data fetch percentage accross datasets, except for Fibonacci dataset. Next, individual average data fetch and chart rendering results are presented.

\addfigure[width=0.9\columnwidth, alt={Comparison of avergage rendering time, presented at the top, and percent increase from the lowest rendering time, presented at the bottom, across seven data management configurations for conditional range queries. ESeMan configurations outperform alternatives in rendering time.}]{figs/cond_total_drawing_relative_increase.png}{Comparison of avergage rendering time (top) and percent increase from the lowest rendering time (bottom) across seven data management configurations for conditional range queries. ESeMan configurations outperform alternatives in rendering time.}{fig:crangeqc}

Data fetch Performance for conditional queries is shown in \autoref{fig:crangeqb}. ESeMan again maintained latency within the interactivity threshold across all datasets, while alternative approaches often exceeded it by large margins (top). Similarly, The \texttt{Agg} variant also crashed on the Fibonacci dataset for this query due to a memory allocation error. Visual accuracy results (bottom) similarly favored ESeMan, with SSIM values approaching one, confirming that the ESeManc configurations preserve both efficiency and visual accuracy for conditional range queries.

\autoref{fig:crangeqc} shows chart rendering times for the conditional range query. Though the ESeMan configurations exceed the interactivity threshold for larger datasets in terms of chart rendering time (top), they still have similar performance compared to other configurations while maintaining high visual accuracy. However, the \texttt{Agg} variant crashed on the Synthesized dataset while rendering the chart due to a memory error. The percent increase of average rendering time relative to the lowest (bottom) shows similar trends to range queries, with ESeMan configurations exhibiting lower percent increase compared to others. Faster rendering time for fibonacci dataset using ESeMan configurations, lower than the data fetch time (\autoref{fig:crangeqb}(top)), contributes toward higher data fetch percentage in \autoref{fig:crangeqa}.

\addfigure[width=0.8\columnwidth, alt={Total memory consumption across datasets. ESeMan configurations remain efficient, while DuckDB-based approaches show higher usage.}]{figs/window_memory_comparison.png}{Total memory consumption across datasets. ESeMan configurations remain efficient, while DuckDB-based approaches show higher usage.}{fig:memory}

\addfigure[width=0.8\columnwidth, alt={Accuracy-latency trade-off in ESeMan-1DKDT. Increasing the pixel window reduces fetch time but decreases SSIM.}]{figs/ssim_fetch_combined.png}{Accuracy-latency trade-off in ESeMan-1DKDT. Increasing the pixel window reduces fetch time but decreases SSIM.}{fig:ssim}

\subsubsection{Memory Consumption}
Memory consumption across all configurations is shown in \autoref{fig:memory}. The summed-area table exhibited the lowest footprint, while DuckDB-based configurations consumed the most. ESeMan\'s LMDB-backed variants maintained low and predictable usage, confirming their suitability for interactive workloads at scale.

\addfigure[width=\columnwidth, alt={Parallel timeline charts constructed from 160K events distributed across 49 processors (rows) of a parallel program. Gray boxes with dark borders represent single event. The top-left figure shows a naive rendering of all data, the other figures show renderings using ESeMan with varying summarization levels. The naive solution requires 499 milliseconds (ms) for the data fetch, which is outside best practices for interactivity. ESeMan retrieves data for an equivalent chart in one tenth the time and can be tuned to trade off accuracy for fetch time. In the bottom-left figure, a 16-pixel summarization window reduces fetch time by half with noticeable visual differences (quantified by the SSIM score). In the bottom-right figure, a 32-pixel window further decreases fetch time with further degradation to accuracy.}]{figs/esemanteaser.png}{Parallel timeline charts constructed from 160K events distributed across 49 processors (rows) of a parallel program. Gray boxes with dark borders represent single event. (a) shows a naive rendering of all data, (b - d) show renderings using ESeMan with varying summarization levels. The naive solution requires 499 milliseconds (ms) for the data fetch, which is outside best practices for interactivity. ESeMan retrieves data for an equivalent chart in one tenth the time and can be tuned to trade off accuracy for fetch time. In (c), a 16-pixel summarization window reduces fetch time by half with noticeable visual differences (quantified by the SSIM score). In (d), a 32-pixel window further decreases fetch time with further degradation to accuracy.}{fig:teasers}

\subsection{Accuracy and Latency Trade-Offs}

To explore tunability, we varied pixel window sizes for \texttt{ESeMan-1DKDT}. \autoref{fig:ssim} illustrates the trade-off between fetch time and SSIM across the three largest datasets. Narrower windows preserved perfect accuracy but incurred longer latencies, while wider windows reduced latency at the cost of structural fidelity. \autoref{fig:teasers} further shows rendered outputs for the LRA dataset, highlighting the effect of summarization window sizes. These results demonstrate that ESeMan allows users to balance responsiveness and accuracy according to analytic needs.

\addfigure[width=\columnwidth, alt={Description of the naive range query.}]{figs/listings51.png}{C3 - Naive range query.}{fig:listings51}

\addfigure[width=\columnwidth, alt={Description of the statistical subsampling query.}]{figs/listings52.png}{C4 - Statistical subsampling query.}{fig:listings52}

\subsection{Comparison with Perfetto}
We qualitatively compared ESeMan with Google\'s \emph{Perfetto}~\cite{Perfetto} using the K-Means dataset. Summarization in Perfetto omits fine-grained events at low zoom levels to achieve responsiveness, potentially concealing short-duration events crucial for anomaly detection~\cite{guo2021survey}. ESeMan, in contrast, preserves pixel-level accuracy across zoom levels.

In addition with better visual accuracy, ESeMan demonstrated comparable responsiveness, with interaction-to-next-paint (INP~\cite{inp_webdev}) values of 23ms, closely matching 20ms of Perfetto. ESeMan's comparable responsiveness to Perfetto highlights that high accuracy need not come at the cost of interactive performance.

\addfigure[width=\columnwidth, alt={Description of the M4 optimization query.}]{figs/listings53.png}{C5 - M4 optimization query.}{fig:listings53}
...
The naive range query presented in \autoref{fig:listings51} selects raw trace entries within a specified time range without any optimization. For statistical subsampling presented in \autoref{fig:listings52}, we use reservoir sampling to reduce data volume while preserving statistical characteristics. Finally, the M4 optimization query presented in \autoref{fig:listings53} uses aggregation over fixed-size time bins, returning representative min/max pairs.

\section{Dataset Indexing Time}

\addtable{Indexing time for each dataset across data management configurations.}{tab:indexing-results1}{|l|c|c|p{2.5cm}|c|}{
  HLINE
  BOLD Dataset & BOLD ESeMan-1DKDT & BOLD ESeMan-KDT & BOLD Summed-Area Table & BOLD DuckDB \\
  HLINE
...
\addtable{Indexed file size for each dataset across data management configurations.}{tab:indexing-results}{|l|c|c|p{2.5cm}|c|}{
  HLINE
  BOLD Dataset & BOLD ESeMan-1DKDT & BOLD ESeMan-KDT & BOLD Summed-Area Table & BOLD DuckDB \\
  HLINE
...
Dataset indexing refers to the time required to ingest and preprocess trace data before the data becomes available for interactive use within the Traveler ecosystem. The dataset indexing step includes parsing the trace file, extracting event metadata, and storing it in the appropriate backend (LMDB, MySQL, or DuckDB), depending on the configuration.

Table~\ref{tab:indexing-results1} presents the dataset indexing time. For the dataset indexing time, ESeMan-based configurations do have a higher construction time compared to the other configurations for the larger datasets.

Table~\ref{tab:indexing-results} presents the indexed dataset size. The indexed dataset size is lower with the smaller datasets for the ESeMan configurations compared to others, and the dataset size is larger for the large dataset sizes compared to the other configurations.

\addfigure[width=\columnwidth, alt={Initial average data fetch time across datasets.}]{figs/window_data_fetch_first_comparison.png}{Initial average data fetch time across datasets.}{fig:initial-render-time}

Note that, DuckDB-based configurations share identical indexing pipelines, including the JSON conversion, and thus report the same values. ESeMan-Agg is in-memory; therefore, it is not reported in Table~\ref{tab:indexing-results1} and Table~\ref{tab:indexing-results}. Synthesized data followed a different pipeline to construct the summed-area table and DuckDB. Therefore, its construction time is also not reported in Table~\ref{tab:indexing-results1} as it will not be a fair comparison.

\section{Initial Data Fetch Time}

Initial data fetch time measures how long it takes to fetch the data for the first visual view of a trace once it is loaded. The initial rendering time reported in our experiments does not include the startup time of the visualization server itself. Instead, it is measured from the point after the Chromium browser is launched and the visualization interface is loaded via its URL. The initial data fetch time captures only the duration required for the client-side application to request and render the initial view after the server starts.

\autoref{fig:initial-render-time} presents the average data fetch time for the initial rendering. Although for the smaller datasets DGEMM and K-Means, the data fetch time is within the interactivity threshold limit of 100 milliseconds, all configurations take a very long time to render the visualization with a larger dataset. However, we notice ESeMan-Agg constantly requires less time to load across all datasets. Due to the sparse distribution of data, ESeMan-KDT takes comparatively longer time with the Synthesized dataset. For visual accuracy, ESeMan-1DKDT and ESeMan-Agg consistently performed best compared to others across all datasets. The statistical subsampling has the worst accuracy among the configurations.

\addfigure[width=\columnwidth, alt={Generated pixel wise differences while visualizing D2 using the summed-area table configuration considering C3 as the baseline. Pixel-wise differences are highlighted using a colormap. SSIM is 0.9996}]{figs/summed_area_ssim_diff.png}{Generated pixel wise differences while visualizing D2 using the summed-area table configuration considering C3 as the baseline. Pixel-wise differences are highlighted using a colormap. SSIM = 0.9996}{fig:ssim-results-summed}

\addfigure[width=\columnwidth, alt={Generated pixel wise differences while visualizing D2 using the statistical subsampling configuration considering C3 as the baseline. Pixel-wise differences are highlighted using a colormap. SSIM is 0.9915}]{figs/db_duck_sketch_ssim_diff.png}{Generated pixel wise differences while visualizing D2 using the statistical subsampling configuration considering C3 as the baseline. Pixel-wise differences are highlighted using a colormap. SSIM = 0.9915}{fig:ssim-results-statistical}

\addfigure[width=\columnwidth, alt={Generated pixel wise differences while visualizing D2 using the M4 optimization configuration considering C3 as the baseline. Pixel-wise differences are highlighted using a colormap. SSIM is 0.8908}]{figs/db_duck_min_max_ssim_diff.png}{Generated pixel wise differences while visualizing D2 using the M4 optimization configuration considering C3 as the baseline. Pixel-wise differences are highlighted using a colormap. SSIM = 0.8908}{fig:ssim-results-m4}
...
The experimental results highlight the effectiveness of \textbf{ESeMan} as a data management strategy for parallel timeline charts. Among the techniques compared, ESeMan consistently provided the fastest query response times while maintaining accuracy nearly indistinguishable from the baseline naive method. Our results demonstrate that a purpose-built hierarchical approach can simultaneously deliver high efficiency and high accuracy, a combination not consistently achieved by alternative strategies.  
