...
\addfigure[width=1.0\columnwidth]{figs/ganttoverview.png}{Illustration of a parallel timeline chart showing three tracks of event sequences across a selected time window.}{fig:eseman_gantt_desc}
...
\addfigure[width=\columnwidth]{figs/framework.png}{ESeMan pipeline. Raw events are cleaned and serialized, then indexed hierarchically across time and tracks. A query server responds to requests from the visualization, returning summarized events that are sufficient to render at the current scale. The pixel window parameter adjusts the visual accuracy and latency.}{fig:pipeline}
...
\addfigure[width=0.8\columnwidth]{figs/agglokdt.png}{Hierarchical organizations for a single track. During queries, once a node corresponds to the spatial-temporal footprint of a pixel window, the node's summary (not its individual events) is returned, yielding a resolution-appropriate depiction.}{fig:hierarchy}

\addfigure[width=\columnwidth]{figs/2dkdt-02.png}{Two-dimensional KD-tree spanning time and tracks. Internal nodes summarize multi-track regions; when a pixel window maps to a node containing multiple events, only the node's temporal (and track) bounds are returned, forming a faithful summary mark.}{fig:twodkdt}
...
\addfigure[width=0.9\columnwidth]{figs/window_data_fetch_comparison.png}{Comparison of average fetch time (top) and visual accuracy (bottom) across seven data management configurations for range queries. The dashed line marks the 100ms interactivity threshold. ESeMan configurations reduce average fetch time while preserving high visual accuracy.}{fig:rangeqb}
...
\addfigure[width=0.9\columnwidth]{figs/window_total_drawing_relative_increase.png}{Comparison of avergage rendering time (top) and percent increase from the lowest rendering time (bottom) across seven data management configurations for range queries. ESeMan configurations outperform alternatives in rendering time.}{fig:rangeqc}
...
\addfigure[width=0.9\columnwidth]{figs/cond_data_fetch_comparison.png}{Comparison of average fetch time (top) and visual accuracy (bottom) across seven data management configurations for conditional range queries. The dashed line marks the 100ms interactivity threshold. ESeMan configurations reduce average fetch time while preserving high visual accuracy.}{fig:crangeqb}
...
\addfigure[width=0.9\columnwidth]{figs/cond_total_drawing_relative_increase.png}{Comparison of avergage rendering time (top) and percent increase from the lowest rendering time (bottom) across seven data management configurations for conditional range queries. ESeMan configurations outperform alternatives in rendering time.}{fig:crangeqc}
...
\addfigure[width=0.8\columnwidth]{figs/window_memory_comparison.png}{Total memory consumption across datasets. ESeMan configurations remain efficient, while DuckDB-based approaches show higher usage.}{fig:memory}

\addfigure[width=0.8\columnwidth]{figs/ssim_fetch_combined.png}{Accuracy-latency trade-off in ESeMan-1DKDT. Increasing the pixel window reduces fetch time but decreases SSIM.}{fig:ssim}
...
\addfigure[width=\columnwidth]{figs/listings51.png}{C3 - Naive range query.}{fig:listings51}

\addfigure[width=\columnwidth]{figs/listings52.png}{C4 - Statistical subsampling query.}{fig:listings52}
...
\addfigure[width=\columnwidth]{figs/listings53.png}{C5 - M4 optimization query.}{fig:listings53}
...
\addtable{Indexing time for each dataset across data management configurations.}{tab:indexing-results1}{|l|c|c|p{2.5cm}|c|}{
  HLINE
  BOLD  & BOLD ESeMan-1DKDT & BOLD ESeMan-KDT & BOLD Summed Area Table & BOLD DuckDB \\
  HLINE
...
\addfigure[width=\columnwidth]{figs/window_data_fetch_first_comparison.png}{Initial average data fetch time across datasets}{fig:initial-render-time}
...
\addtable{SSIM comparison across different configurations visualizing D2. SSIM values are calculated considering C3 as the baseline. Pixel-wise differences are highlighted using a colormap.}{tab:ssim-results}{|c|p{4cm}|}{
  ...
  HLINE
  \includegraphics[width=0.6\columnwidth]{figs/faf17535-2f66-4621-995f-49c7dbd84e8b_window_summed_area_table_1_gantt_diff.png} & Summed Area Table (0.9996) \\ HLINE
  \includegraphics[width=0.6\columnwidth]{figs/faf17535-2f66-4621-995f-49c7dbd84e8b_window_db_duck_sketch_1_gantt_diff.png} & Statistical subsampling (0.9915) \\ HLINE
  \includegraphics[width=0.6\columnwidth]{figs/faf17535-2f66-4621-995f-49c7dbd84e8b_window_db_duck_min_max_1_gantt_diff.png} & M4 Optimization (0.8908) \\ HLINE
  \includegraphics[width=0.6\columnwidth]{figs/hotmap.png} & \\ HLINE
}
